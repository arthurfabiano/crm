...
class Omni extends Omnipay {
	...
	function __construct() {
		...
		$payment_paypal = $this->CI->Settings_Model->payment_mode('paypal');
		$payment_authorize = $this->CI->Settings_Model->payment_mode('authorize');

		// AUTHORIZE.NET AIM SETTINGS

		$this->merchanAuthentication = new net\ authorize\ api\ contract\ v1\ MerchantAuthenticationType();
		$this->merchanAuthentication->setName( $payment_authorize['input_value1'] );
		$this->merchanAuthentication->setTransactionKey( $payment_authorize['input_value2'] );
		$this->refId = 'ref' . time();
		...
		$sanbox = $payment_paypal['sandbox_account'];
		...
		$this->add_field( 'business', $payment_paypal['input_value1'] ); 
		$this->add_field( 'rm', '2' ); // Return method = POST
		$this->add_field( 'cmd', '_xclick' );
		$this->add_field( 'currency_code', $payment_paypal['input_value2'] );
	...
	function stripe_success($data) {
		try {
			$payment = $this->CI->Settings_Model->payment_mode('stripe');
			//$this->CI->Settings_Model->get_payment_gateway_data();
		    $gatewayStripe = Omnipay::create('Stripe');
			$gatewayStripe->setApiKey($payment['input_value1']);
			if ($payment['sandbox_account'] == '1') {
