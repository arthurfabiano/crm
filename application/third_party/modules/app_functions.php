...
function aplCheckSettings()
    ...
    if (APL_STORAGE=="FILE" && is_file(APL_DIRECTORY."/".APL_LICENSE_FILE_LOCATION)) {
        if (!@is_writable(APL_DIRECTORY."/".APL_LICENSE_FILE_LOCATION)) {
            $notifications_array[]=APL_CORE_NOTIFICATION_INVALID_LICENSE_FILE;
        }
    } else {
        if (APL_STORAGE=="FILE") {
            if(ini_get('allow_url_fopen')) {
                $fp = fopen(APL_DIRECTORY."/".APL_LICENSE_FILE_LOCATION,"wb");
                fclose($fp);
            } else {
                $notifications_array[]=APL_CORE_NOTIFICATION_INVALID_LICENSE_FILE;
            }
...
function aplVerifyLicense($MYSQLI_LINK=null, $FORCE_VERIFICATION=0)
    ...
    function remote_check($MYSQLI_LINK=null)
    {
    $notifications_array=array();
    if (empty($apl_core_notifications=aplCheckSettings()))
        {
            ...
            if ($notifications_array['notification_case']=="notification_license_ok")
                {
                    $handle=@fopen(APL_DIRECTORY."/".APL_LICENSE_FILE_LOCATION, "w+");
                    @fclose($handle);
                    $notifications_array['notification_case'] = "remote_check_done";
                    $notifications_array['notification_text'] = "remote_check_done";
                }
            }
        else
            {
